...
if __name__ == "__main__":
    ...
    net.load_state_dict(_pickle.load(open(args.ckpt_path, 'rb'))[0])

    snake = SnakePytorch(args.delta, args.batch_sz, args.num_lines, args.radius)

    print("eval...")
    # eval with perfect center
    ...
    for seed in seeds:
        ...
        dice, dice_inside_bboxes = \
            do_eval(net, snake, valid.images, valid.masks, valid.centers, valid.bboxes, args.batch_sz, args.num_lines,
                    args.radius, smoothing_window=args.smoothing_window, jitter_radius=valid.jitter_radius,
                    jitter_radius_ratio=args.jitter_radius_ratio)
        dices.append(dice)
    dices = np.asarray(dices)
    print(f"jitter dice mean: {np.mean(dices)}")
    print(f"jitter dice std: {np.std(dices)}")
    print(f"jitter dice min: {np.min(dices)}")
    print(f"jitter dice max: {np.max(dices)}")
